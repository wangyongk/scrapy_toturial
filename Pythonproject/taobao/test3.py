...
price1_count = (np.array(prices) < 30).sum()
price2_count = (np.array(prices) < 100).sum() - price1_count
price3_count = (np.array(prices) < 200).sum() - price1_count - price2_count
price4_count = (np.array(prices) >= 200).sum()
prices_stat = pd.DataFrame({'价格': ['小于30元', '30-100元之间', '100-200元之间', '200元以上'], '数量': [price1_count, price2_count, price3_count, price4_count]})
